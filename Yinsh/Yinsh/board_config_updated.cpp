...
// using namespace std; 
        ...
        // 1 Me
            ...
            if (results.at(0).compare("S")==0)       // Select ring
            {
                ...
                if (n>=7)   // select and move ring & remove discs and ring
                {
                    ...
                    rings_removed.push_back(ring_x);
                    rings_removed.push_back(ring_y);
    ...
    std::string Board::anything_to_remove_before_proceeding(){
    



                std::string s_d = "";
                std::vector<int> v = find_five_consecutive_board_array(0);
                // std::cerr << "this is after 5 consec check"; 
                print();
                ...
                if (v.size()!=0)
                {
                                ...
                                s_d.append("X");
                                    ...
                                    v_prime = find_which_ring_to_remove_board_array(0);
                                    print();
                    ...
                    board_array[ring_x][ring_y] = 0;
                    rings_removed.push_back(ring_x-15);
                    rings_removed.push_back(ring_y-15);
                }
    ...
    std::string Board::play() // returns the move to be played
    {
        ...
        {    
            std::string to_add_to_move_before = anything_to_remove_before_proceeding();
            ...
            // print();
            update(-1,move,0);
            // std::cerr << "this is after 1 st and before 2nd proceed check"; 
            print();

            std::string to_add_to_move_after = anything_to_remove_before_proceeding();
...
// 
    ...
    std::vector<int> Board::find_which_ring_to_remove_board_array(int ht){
        std::vector<int> v;
        ...
        for (int i = 0; i < 5; ++i)
        {
            for (int j = 0; j < rings_removed.size(); j=j+2)
            {
                if (computers_ring[2*i]==rings_removed.at(j) && computers_ring[2*i+1]==rings_removed.at(j+1) ){
                    flag = true;
                }
            ...
            if (flag==true){
                ...
                int to_re_update = -3;
                int ring_x = computers_ring[2*i];
                int ring_y = computers_ring[2*i+1];
                to_re_update =  board_array[ring_x][ring_y] ;
                board_array[ring_x][ring_y] = 0 ;
                int score = evaluate_heuristic();
                if (to_re_update == 2){
                    score = -score;
                ...
                board_array[ring_x][ring_y] = to_re_update;
            }
    ...
    int Board::how_good_disc_is(int x,int y){
        int attack =0;
        bool ring =false;
        int max_X[] = {3,6,7,8,9,8,9,8,7,6,3};
        bool empty = false;
    ...
    int Board::my_ring_on_blank(int x,int y){
        int max_X[] = {3,6,7,8,9,8,9,8,7,6,3};
        int attack =0;
        bool ring =false;
        bool empty = false;
    ...
    int Board::how_good_ring_is(int x,int y){
        int attack =0;
        int max_X[] = {3,6,7,8,9,8,9,8,7,6,3};
        int my_disc =0;
        int other_disc = 0;
    ...
    int Board::eval_five(int b1,int a1,int a2,int a3,int a4,int a5,int b2,int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4,int x5,int y5)
    {
        ...
        if(disc==4&&(other_disc==1||blank==1||ring==1)){
            ...
            if(temp_board_array[x1][y1]==-1){
                my_disc_can_convert_by_other+=how_good_disc_is(x1,y1);
            }
            if(temp_board_array[x2][y2]==-1){
                my_disc_can_convert_by_other+=how_good_disc_is(x2,y2);
            }
            if(temp_board_array[x3][y3]==-1){
                my_disc_can_convert_by_other+=how_good_disc_is(x3,y3);
            }
            if(temp_board_array[x3][y3]==-1){
                my_disc_can_convert_by_other+=how_good_disc_is(x4,y4);
            }
            if(temp_board_array[x4][y4]==-1){
                my_disc_can_convert_by_other+=how_good_disc_is(x5,y5);
            }
        ...
        if(disc==4 && other_ring==1){
            ...
            if(temp_board_array[x1][y1]==2){
                my_disc_can_convert_by_other+=how_good_ring_is(x1,y1);
            }
            if(temp_board_array[x2][y2]==2){
                my_disc_can_convert_by_other+=how_good_ring_is(x2,y2);
            }
            if(temp_board_array[x3][y3]==2){
                my_disc_can_convert_by_other+=how_good_ring_is(x3,y3);
            }
            if(temp_board_array[x3][y3]==2){
                my_disc_can_convert_by_other+=how_good_ring_is(x4,y4);
            }
            if(temp_board_array[x4][y4]==2){
                my_disc_can_convert_by_other+=how_good_ring_is(x5,y5);
            }
            result-=my_disc_can_convert_by_other*200;
        }
        ...
        if(disc==4&&blank==1){
            if(temp_board_array[x1][y1]==0){
                my_disc_can_convert_by_other+=my_ring_on_blank(x1,y1);
            }
            if(temp_board_array[x2][y2]==0){
                my_disc_can_convert_by_other+=my_ring_on_blank(x2,y2);
            }
            if(temp_board_array[x3][y3]==0){
                my_disc_can_convert_by_other+=my_ring_on_blank(x3,y3);
            }
            if(temp_board_array[x3][y3]==0){
                my_disc_can_convert_by_other+=my_ring_on_blank(x4,y4);
            }
            if(temp_board_array[x4][y4]==0){
                my_disc_can_convert_by_other+=my_ring_on_blank(x5,y5);
            }
    ...
    int Board::each_ring(int x, int y){
        ...
        if(temp_board_array[x][y]==-1){
            my_disc_can_convert_by_other+=how_good_disc_is(x,y);
        }
        if(temp_board_array[x][y]==1){
            other_disc_can_convert_to_my+=how_good_disc_is(x,y);
        }
        if(temp_board_array[x][y]==-2){
            my_ring_good+=how_good_ring_is(x,y);
        }
        if(temp_board_array[x][y]==2){
            other_ring_good+=how_good_ring_is(x,y);
        }
        else{
            blank++;
        }
    ...
    int Board::evaluate_heuristic(){
     int result = 0;
        ...
        int max_X[] = {6,7,8,9,8,9,8,7,6};
        // Y = 0;
        int max_X2[]={3,6,7,8,9,8,9,8,7,6,3};
        for(int i = - 5;i<=5;i++){
            int y_m = max_X2[i+5];
            for(int j = -y_m;j<=y_m;j++){
                each_ring(j,i);
            }
        }
        for (int i = -4; i < 5; i++) // y axis
        {
            ...
            {   
                ...
                result = result + eval_five(b1,temp_board_array[15+j][15+i],temp_board_array[15+j+2][15+i],temp_board_array[15+j+4][15+i],temp_board_array[15+j+6][15+i],temp_board_array[15+j+8][15+i],b2,15+j,15+i,15+j+2,15+i,15+j+4,15+i,15+j+6,15+i,15+j+8,15+i);
                result = result - eval_five(-b1,-temp_board_array[15+j][15+i],-temp_board_array[15+j+2][15+i],-temp_board_array[15+j+4][15+i],-temp_board_array[15+j+6][15+i],-temp_board_array[15+j+8][15+i],-b2,15+j,15+i,15+j+2,15+i,15+j+4,15+i,15+j+6,15+i,15+j+8,15+i);
        ...
        for (int i = 0; i < 9; ++i) 
        {
            ...
            while(true)
            {
                ...
                result = result + eval_five(b1,temp_board_array[15+x][15+y],temp_board_array[15+x+1][15+y+1],temp_board_array[15+x+2][15+y+2],temp_board_array[15+x+3][15+y+3],temp_board_array[15+x+4][15+y+4],b2,15+x,15+y,15+x+1,15+y,15+x+2,15+y,15+x+3,15+y,15+x+4,15+y+5);
                result = result - eval_five(-b1,-temp_board_array[15+x][15+y],-temp_board_array[15+x+1][15+y+1],-temp_board_array[15+x+2][15+y+2],-temp_board_array[15+x+3][15+y+3],-temp_board_array[15+x+4][15+y+4],-b2,15+x,15+y,15+x+1,15+y,15+x+2,15+y,15+x+3,15+y,15+x+4,15+y+5);
                x = x+1;
                y = y+1;
                if (points_count==in_line[i])
        ...
        for (int i = 0; i < 9; ++i)
        {
            ...
            while(true)
            {
                ...
                result = result + eval_five(b1,temp_board_array[15+x][15+y],temp_board_array[15+x-1][15+y+1],temp_board_array[15+x-2][15+y+2],temp_board_array[15+x-3][15+y+3],temp_board_array[15+x-4][15+y+4],b2,15+x,15+y,15+x-1,15+y+1,15+x-2,15+y+2,15+x-3,15+y+3,15+x-4,15+y+4);
                result = result -eval_five(-b1,-temp_board_array[15+x][15+y],-temp_board_array[15+x-1][15+y+1],-temp_board_array[15+x-2][15+y+2],-temp_board_array[15+x-3][15+y+3],-temp_board_array[15+x-4][15+y+4],-b2,15+x,15+y,15+x-1,15+y+1,15+x-2,15+y+2,15+x-3,15+y+3,15+x-4,15+y+4);
                x = x-1;
                y = y+1;
                if (points_count==in_line[i])
